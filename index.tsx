...
const settings = definePluginSettings({
    ...
    hiddenServers: {
        ...
        component: () => {
            ...
            const isStreamMode = isStreamingMode();
...
function updateCSSClasses() {
    const shouldHide = !settings.store.onlyHideInStream || isStreamingMode();
...
function isServerHidden(serverId: string): boolean {
    return hiddenData.servers.includes(serverId);
}

function isFolderHidden(folderId: string): boolean {
    return hiddenData.folders.includes(folderId);
}

// ==================================
// STREAMER MODE DETECTION FUNCTION =
// ==================================
function isStreamingMode(): boolean {
    try {
        const StreamerModeStore = Vencord.Webpack.findStore("StreamerModeStore");
        if (StreamerModeStore) return StreamerModeStore.enabled;

        const stores = Vencord.Webpack.getByProps("StreamerModeStore");
        if (stores?.StreamerModeStore) return stores.StreamerModeStore.enabled;

        const UserSettingsStore = Vencord.Webpack.findStore("UserSettingsStore");
        return UserSettingsStore?.streamerMode ?? false;
    } catch (e) {
...
export default definePlugin({
    ...
    shouldHideServer(serverId: string): boolean {
        if (!settings.store.onlyHideInStream) return isServerHidden(serverId);
        return isServerHidden(serverId) && this.isStreamingMode();
    },

    shouldHideFolder(folderId: string): boolean {
        if (!settings.store.onlyHideInStream) return isFolderHidden(folderId);
        return isFolderHidden(folderId) && this.isStreamingMode();
    },
    ...
    filterGuildsTree(guildsTree: any) {
        ...
        try {
            ...
            filteredTree.root.children = guildsTree.root.children.filter((node: any) => {
                if (node.type === "folder") {
                    if (isFolderHidden(node.id)) {
                        return false;
                    }

                    if (node.children) {
                        const filteredChildren = node.children.filter((guild: any) =>
                            !isServerHidden(guild.id)
                        );
    ...
    getHiddenServers(): string[] {
        return [...hiddenData.servers];
    },

    getHiddenFolders(): string[] {
        return [...hiddenData.folders];
    },
